...
import com.jonas.demoparkapi.service.ClienteVagaService;
import com.jonas.demoparkapi.service.EstacionamentoService;
import com.jonas.demoparkapi.web.dto.EstacionamentoCreateDto;
import com.jonas.demoparkapi.web.dto.EstacionamentoResponseDto;
import com.jonas.demoparkapi.web.dto.PagebleDto;
import com.jonas.demoparkapi.web.dto.mapper.ClienteVagaMapper;
...
public class EstacionamentoController {
    ...
    public ResponseEntity<EstacionamentoResponseDto> getByReceibo(@PathVariable String recibo){
    ...
    @Operation(summary = "Localizar os registros de estacionamentos do cliente por CPF", description = "Localizar os " +
            ...
            responses = {
                    @ApiResponse(responseCode = "200", description = "Recurso localizado com sucesso",
                            content = @Content(mediaType = " application/json;charset=UTF-8",
                                    schema = @Schema(implementation = PagebleDto.class))),
                    @ApiResponse(responseCode = "403", description = "Recurso não permito ao perfil de CLIENTE",
                            content = @Content(mediaType = " application/json;charset=UTF-8",
                                    schema = @Schema(implementation = ErrorMessage.class)))
            })
    ...
    public ResponseEntity<PagebleDto> getAllEstacionamentosPorCpf(@PathVariable String cpf,
                                                                  ...
                                                                  direction = Sort.Direction.ASC) Pageable pageable){ // size = 5, sort = "dataEntrada", direction = Sort.Direction.ASC significa que a paginação terá 5 elementos por página, ordenados pela data de entrada em ordem crescente
    ...
    public ResponseEntity<PagebleDto> getAllEstacionamentosDoCliente(@AuthenticationPrincipal JwtUserDetails user, // essa anotação pega o usuário autenticado
                                                                     @PageableDefault(size = 5, sort = "dataEntrada",
                                                                          direction = Sort.Direction.ASC) Pageable pageable){ // size = 5, sort = "dataEntrada", direction = Sort.Direction.ASC significa que a paginação terá 5 elementos por página, ordenados pela data de entrada em ordem crescente
